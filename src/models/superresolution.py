...
from models.networks_stylegan2 import Conv2dLayer, FullyConnectedLayer, MappingNetwork, SynthesisBlock, SynthesisLayer, ToRGBLayer
...
class SuperresolutionHybrid8XDC(torch.nn.Module):
    def __init__(self, channels, img_resolution, img_channels, sr_num_fp16_res, sr_antialias,
                 num_fp16_res=4, conv_clamp=None, channel_base=None, channel_max=None,  # IGNORE
                 **block_kwargs):
        super().__init__()
        assert img_resolution == 256
        ...
        self.input_resolution = 64
        self.sr_antialias = sr_antialias
        self.block0 = SynthesisBlock(channels, 256, w_dim=512, resolution=128,
                                     img_channels=img_channels, is_last=False, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.block1 = SynthesisBlock(256, 128, w_dim=512, resolution=256,
    ...
    def forward(self, x, ws, **block_kwargs):
        ...
        x, img = self.block0(x, img=None, ws=ws, **block_kwargs)
        x, img = self.block1(x, img=img, ws=ws, **block_kwargs)
        return img
...
@persistence.persistent_class
class SuperresolutionNetwork(torch.nn.Module):
    def __init__(
        self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        raw_resolution,             # Input image resolution.
        img_resolution,             # Output image resolution.
        raw_channels,               # Number of input channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        self.raw_resolution_log2 = int(np.log2(raw_resolution))
        self.img_resolution = img_resolution
        self.img_resolution_log2 = int(np.log2(img_resolution))
        self.raw_channels = raw_channels
        self.img_channels = img_channels
        self.num_fp16_res = num_fp16_res
        self.block_resolutions = [2 ** i for i in range(self.raw_resolution_log2 + 1, self.img_resolution_log2 + 1)]
        channels_dict = {res: min(channel_base // res, channel_max) for res in self.block_resolutions}
        fp16_resolution = max(2 ** (self.img_resolution_log2 + 1 - num_fp16_res), 8)

        self.num_ws = 0
        for res in self.block_resolutions:
            in_channels = channels_dict[res // 2] if res > raw_resolution * 2 else raw_channels
    ...
    def extra_repr(self):
        return ' '.join([
            ...
            f'raw_resolution={self.raw_resolution:d}, img_resolution={self.img_resolution:d}, raw_channels={self.raw_channels:d}, img_channels={self.img_channels:d},',
...
@persistence.persistent_class
class SuperresolutionXDC(torch.nn.Module):
    def __init__(
        self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.superresolution = SuperresolutionNetwork(w_dim=w_dim, raw_resolution=raw_resolution, img_resolution=img_resolution, input_channels=input_channels, img_channels=img_channels,
                                                      **synthesis_kwargs)
