...
def project(
    ...
    *,
    ...
    # Setup noise inputs.
    noise_bufs = {name: buf for (name, buf) in G.backbone.named_buffers() if 'noise_const' in name}

    _, start_w, _ = G.encode(img)
    # start_w = G.encoder.w_avg[None, None, :].repeat(1, G.num_ws, 1)
    w_opt = start_w.requires_grad_(True)  # pylint: disable=not-callable
    ...
    target_hair = G.sample(target_image, coords, mode='nearest')
    ...
    for step in range(num_steps):
        ...
        ws = w_opt + w_noise
        synth_img = G.synthesis(ws=ws, noise_mode='const')
        synth_image = torch.cat([img['image_raw'], synth_img['image']], dim=1)
        synth_hair = G.sample(synth_image, coords, mode='nearest')
        strand_loss = criterion(synth_hair, target_hair)
        tex_loss = F.l1_loss(img['image'], synth_img['image'])
        ...
        loss = strand_loss['pos'] + strand_loss['rot'] + strand_loss['cur'] + 1.0 * tex_loss + regularize_noise_weight * reg_loss
...
def run_projection(G, target, outdir, hair_roots, num_steps, save_video, device):
    ...
    image_raw = target_image[:, :G.raw_channels]
    image_res = target_image[:, G.raw_channels:]
    image_mask = target_data['mask'].unsqueeze(0)
    target_rgb = image_to_rgb(image_res[0], rgb_size=256)
    roots = target_data['roots']
    coords = hair_roots.rescale(roots[..., :2]).unsqueeze(0)
    img = {'image': image_res, 'image_mask': image_mask, 'image_raw': image_raw}
    ...
    if save_video:
        ...
        for projected_w in projected_w_steps[::3]:
            synth_image = G.synthesis(ws=projected_w.unsqueeze(0), noise_mode='const')['image']
    ...
    np.savez(f'{outdir}/{fname}.npz', texture=c2c(synth_image[0]), roots=c2c(roots), w=c2c(projected_w))
    target_hair = G.sample(target_image, coords, mode='nearest')[0]
    ...
    synth_image = torch.cat([image_raw, synth_image], dim=1)
    synth_hair = G.sample(synth_image, coords, mode='nearest')[0]
...
def main(
    ...
    with dnnlib.util.open_url(network_pkl) as f:
        G = legacy.load_network_pkl(f)['G'].requires_grad_(False).to(device)  # type: ignore
    ...
    if os.path.isdir(target):
        target_files = sorted(glob.glob(os.path.join(target, 'high-res', '*.npz')))
