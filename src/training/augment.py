...
class AugmentPipe(torch.nn.Module):
    def __init__(self,
                 ...
                 imgfilter=0, imgfilter_bands=[1, 1, 1, 1], imgfilter_std=1, imgrescale=0,
        ...
        # Image-space random rescaling.
        self.imgrescale = float(imgrescale)             # Probability multiplier for image-space random rescaling.
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if self.imgrescale > 0:
            downsample_size = int(np.random.uniform(low=height // 4, high=height))
            images_low_res = torch.nn.functional.interpolate(images, size=(downsample_size, downsample_size), mode='bilinear', align_corners=False, antialias=True)
            images = torch.nn.functional.interpolate(images_low_res, size=(height, width), mode='bilinear', align_corners=False, antialias=True)
